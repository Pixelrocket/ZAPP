...
local function lpmrequire (main, luarequire)
  ...
  return function (module)
    ...
    local function try (path)
      ...
      -- -- otherwise, fetch the "main" entry from package.json, and load that one
      -- dirpath = ""
      -- if path then dirpath = string.gsub(path, "%.", dirsep) .. dirsep end
      -- local jsonpath = maindir .. dirsep .. dirpath .. "node_modules" .. dirsep .. module .. dirsep .. "package.json"
      ...
      --   mainentry = readtable.main
      -- end
      -- if mainentry then
      --   return callrequire(prefix .. "node_modules." .. module .. "." .. mainentry)
      -- end
      -- just couldn't find it here
      return false
    end

    -- try to load it from just here in the source tree
    if try() then return _M end

    -- walk up the source tree and try to find it somewhere up there
